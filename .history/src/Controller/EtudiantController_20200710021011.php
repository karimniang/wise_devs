...
class EtudiantController extends AbstractController
{
    ...
    public function create(Request $request, EntityManagerInterface $manager)
    {
        ...
        if ($formEtudiant->isSubmitted()) {
            $etud = $formEtudiant->get("Est_Loger")->getViewData();
            //dump($etud);
            ...
            if (!empty($etudiant->getMontant())) {
                ...
                if ($etud == "oui") {
                    $etudiant->setIsHoused(true);
                } elseif ($etud == "non") {
                    $etudiant->setIsHoused(false);
                }
    ...
    /**
     * @Route("/etudiantup/{id<[0-9]+>}/update", name="etudiant_update", methods={"POST","GET"})
     */
