...
class ChambreController extends AbstractController
{
    ...
    public function lister(ChambreRepository $chambreRepository, EtudiantRepository $etudiantRepository)
    {
        ...
        /*for ($i = 0; $i < count($etudiants); $i++) {
            ...
            for ($j = 0; $j < count($chambres); $j++) {
                if ($etudiants[$i]->getLoger() !== null && (($chambres[$j]->getId() == $etudiants[$i]->getLoger()->getId()))) {
                    //dd($etudiants[$i]->getLoger()->getId());
                    //$this->addFlash('succes', 'chambre contient');
                    $message = "Cette chambre contient des etudiants";
                } else {
                    $message = "Cette chambre ";
                }
            }
    ...
    public function deleteChambre(EntityManagerInterface $manager, Chambre $chambre, EtudiantRepository $etudiantRepository)
    {
        ...
        for ($i = 0; $i < count($etudiants); $i++) {
            //dump($etudiants[$i]->getPrenom());
            if ($etudiants[$i]->getLoger() !== null) {
                if ($chambreId == $etudiants[$i]->getLoger()->getId()) {
                    //dd($etudiants[$i]->getLoger()->getId());
                    //$this->addFlash('succes', 'chambre contient');
                    $message = "Cette chambre contient des etudiants";
                } else {
